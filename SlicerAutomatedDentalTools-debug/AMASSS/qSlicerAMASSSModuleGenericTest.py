...
class qSlicerAMASSSModuleGenericTest(unittest.TestCase):

  def setUp(self):
    import slicer
    self.assertTrue(hasattr(slicer, 'modules'))
    self.assertTrue(hasattr(slicer.modules, 'amasss'))
    self.module = slicer.modules.amasss

  def test_file_attribute(self):
    self.assertRegexpMatches(__file__, r'qSlicerAMASSSModuleGenericTest.py[c]?$')

  def test_name_attribute(self):
    self.assertEqual(__name__, 'qSlicerAMASSSModuleGenericTest')

  def test_module_valid(self):
    self.assertNotEqual(self.module, None)
    self.assertEqual(self.module.name, 'AMASSS')
